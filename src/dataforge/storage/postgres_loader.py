...
def load_silver_to_postgres():                                                                  
    ...
    orders_path = PROCESSED_DATA_DIR / "silver" / "field_orders" / "ordens_consolidados.parquet"
    if orders_path.exists():                                                                    
        ...
        log.info(f"Tabela 'raw_silver.field_orders' carregada com {len(df_orders)} linhas.")    
    else:                                                                                       
        log.warning(f"Arquivo não encontrado: {orders_path}")                                   
                                                                                                    
    # 3. Carrega os Clientes da API da Silver                                                   
    customers_path = PROCESSED_DATA_DIR / "silver" / "customers" / "customers_data.parquet"     
    if customers_path.exists():                                                                 
        ...
        log.info(f"Tabela 'raw_silver.customers' carregada com {len(df_customers)} linhas.")    
    else:                                                                                       
